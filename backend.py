...
# 依赖注入函数，用于获取 graph
async def get_dependencies() -> StateGraph:
    """
    依赖注入函数，用于获取 graph。
    ...
    Raises:
        HTTPException: 如果 graph 未初始化，则抛出 500 错误。
    """
    if not graph:
...
async def chat_completions(request: ChatCompletionRequest, dependencies: StateGraph = Depends(get_dependencies)):
    """接收来自前端的请求数据进行业务的处理。
